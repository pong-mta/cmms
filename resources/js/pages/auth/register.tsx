...
import { Building2, CheckCircle2, ChevronDown, LoaderCircle, ShieldCheck } from 'lucide-react';
    ...
    useEffect(() => {
        const loadDepartments = async () => {
            try {
                ...
                setGeneralError('Unable to load departments. Please refresh the page.');
            } finally {
            ...
            if (axios.isAxiosError(error) && error.response?.status === 422) {
                ...
                if (validationErrors) {
                    setErrors({
                        ...
                        department_id: validationErrors.department_id?.[0],
                        password: validationErrors.password?.[0],
                        password_confirmation: validationErrors.password_confirmation?.[0],
                    });
                } else {
                    setGeneralError(error.response.data.message || 'Please check your information.');
                }
                        ...
                        <section className="relative hidden overflow-hidden bg-slate-950 p-8 text-white lg:flex lg:flex-col lg:justify-between xl:p-10">
                            ...
                            <div className="relative">
                                ...
                                <h2 className="mt-2 text-3xl leading-tight font-bold tracking-tight xl:text-4xl">
                                    {' '}
                                    Municipal <span className="block text-blue-400">Operations</span> <span className="block">Platform</span>{' '}
                                </h2>{' '}
                                <p className="mt-4 max-w-sm text-xs leading-6 text-slate-300 xl:mt-6 xl:text-sm xl:leading-7">
                                    {' '}
                                    An integrated platform for managing municipal operations, services, resources, personnel, assets, and field
                                    activities.{' '}
                                </p>{' '}
                                {/* Features */}{' '}
                                <div className="mt-6 space-y-3 xl:mt-8 xl:space-y-4">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <span className="text-[11px] text-slate-300 xl:text-xs"> Municipal Services & Operations </span>{' '}
                                    </div>{' '}
                                    <div className="flex items-center gap-3">
                                        {' '}
                                        <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-blue-500/10">
                                        ...
                                        <span className="text-[11px] text-slate-300 xl:text-xs"> Resources & Asset Management </span>{' '}
                                    </div>{' '}
                                    <div className="flex items-center gap-3">
                                        {' '}
                                        <div className="flex h-7 w-7 items-center justify-center rounded-lg bg-blue-500/10">
                            ...
                            <div className="relative">
                                ...
                                <div className="flex items-center gap-2 text-xs text-slate-400">
                                    {' '}
                                    <ShieldCheck className="h-4 w-4 text-emerald-400" /> <span>Secure municipal access</span>{' '}
                                </div>{' '}
                        ...
                        <section className="overflow-hidden p-5 sm:p-6 lg:p-7">
                            ...
                            <div className="mb-4 xl:mb-5">
                                ...
                                <p className="mt-2 text-xs leading-5 text-slate-500 xl:text-sm">
                                    Register your municipal account to access the maintenance management system.
                                </p>
                            ...
                            <form onSubmit={submit} className="space-y-3.5">
                                ...
                                <div>
                                    <Label htmlFor="department_id" className="mb-1.5 block text-xs font-semibold text-slate-700">
                                        Department / Office
                                    </Label>
                                    ...
                                    <div className="relative">
                                        <select
                                            ...
                                            disabled={processing || loadingDepartments}
                                            className="h-10 w-full appearance-none rounded-xl border border-slate-200 bg-slate-50 px-3 pr-10 text-sm text-slate-700 transition outline-none focus:border-blue-500 focus:bg-white focus:ring-4 focus:ring-blue-50 disabled:cursor-not-allowed disabled:opacity-60"
                                        >
                                            <option value="">{loadingDepartments ? 'Loading departments...' : 'Select department / office'}</option>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <div>
                                        <Label htmlFor="password_confirmation" className="mb-1.5 block text-xs font-semibold text-slate-700">
                                            Confirm Password
                                        </Label>
                                ...
                                <div className="flex items-center gap-3 rounded-xl border border-blue-100 bg-blue-50/70 px-3 py-2.5">
                                    ...
                                    <div>
                                        <p className="text-[11px] font-semibold text-blue-900">Mobile verification required</p>

                                        <p className="text-[10px] leading-4 text-blue-700">
                                            Your mobile number will be verified through OTP after registration.
                                        </p>
                                    </div>
                                </div>
                                ...
                                <Button
                                    type="submit"
                                    disabled={processing || loadingDepartments || departments.length === 0}
                            ...
                            <div className="mt-4 text-center text-xs text-slate-500">
                                ...
                                <TextLink href={route('login')} className="font-semibold text-blue-700 hover:text-blue-800">
                                    Sign in
                                </TextLink>
                            </div>
